...
class RigidPerturb:
    ...
    def _sample_pose_large():
        # anglez = np.random.rand(1) * np.pi * 2  # anglez, angley, anglex
        # angley = np.random.rand(1) * np.pi * 2  # anglez, angley, anglex
        # anglex = np.random.rand(1) * np.pi * 2  # anglez, angley, anglex

        # euler_ab = np.array([anglez[0], 0, 0])
        # rot_ab = Rotation.from_euler('zyx', euler_ab).as_matrix()
        # print(rot_ab.shape)
        
        # trans_x = np.random.uniform(-4,4)
        # trans_y = np.random.uniform(-4,4)
        # trans_z = np.random.uniform(-1,1)
        # # trans_ab = np.random.uniform(-4, 4, (3,1))
        # trans_ab = np.array([trans_x, trans_y, trans_z])

        # print(rot_ab, trans_ab)
        # raise ValueError
        # perturb = np.concatenate([rot_ab, trans_ab], axis=1)
...
class ShufflePoints:
    ...
    def __call__(self, data):
        ...
        # print("data['src_xyz'].shape: ", data['src_xyz'].shape)
        # print("data['src_overlap'].shape: ", data['src_overlap'].shape)
        if data['src_xyz'].shape[0] != data['src_overlap'].shape[0]:
            raise ValueError("data['src_xyz'].shape[0] != data['src_overlap'].shape[0]")
        data['src_xyz'] = data['src_xyz'][src_idx, :]
        data['src_overlap'] = data['src_overlap'][src_idx]
        
        
        # print("data['tgt_xyz'].shape: ", data['tgt_xyz'].shape)
        # print("data['tgt_overlap'].shape: ", data['tgt_overlap'].shape)
        if data['tgt_xyz'].shape[0] != data['tgt_overlap'].shape[0]:
            raise ValueError("data['tgt_xyz'].shape[0] == data['tgt_overlap'].shape[0]")
